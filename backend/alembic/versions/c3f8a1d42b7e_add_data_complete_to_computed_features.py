...
def upgrade() -> None:
    ...
    # Feature versions table — skip if already created by create_all()
    if 'feature_versions' not in existing_tables:
        op.create_table(
            'feature_versions',
    ...
    # New columns on computed_features — skip if already present
    existing_cols = {c['name'] for c in inspector.get_columns('computed_features')}

    if 'data_complete' not in existing_cols:
        op.add_column(
            'computed_features',
    ...
    # Check if the old constraint still exists before trying to swap it.
    existing_constraints = {
        c['name'] for c in inspector.get_unique_constraints('computed_features')
        if c.get('name')
    }
    if 'uq_computed_entry_feature' in existing_constraints:
        with op.batch_alter_table('computed_features') as batch_op:
